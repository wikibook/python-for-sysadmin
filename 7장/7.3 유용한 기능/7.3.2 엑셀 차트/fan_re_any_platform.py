...
# exec_ipmi 대신 exec_ipmi_any_platform 임포트
from exec_ipmi_any_platform import get_ipmi
from exec_ipmi_any_platform import get_server_input
import time
import re
import platform

def check_fan(args) :
    if platform.system() == "Windows" :                 # 윈도에서 실행할 때
        ipmi_result = get_ipmi(" sensor %s" % args)     # 명령어 수정
    else :
...
def get_data_list(pattern_str, source_data) :
    ...
    value_list = []
    for str in str_list :
        # 윈도에서 실행할 때와 분리하여 처리하는 코드로 수정 시작
        if str == "" :
            continue
        if platform.system() == "Windows" :     # 윈도에서 실행할 때
            key = str[0 : str.find("=") ].strip()
            value = str[str.find("=") + 1 : len(str)].strip()
            array = key.split()
